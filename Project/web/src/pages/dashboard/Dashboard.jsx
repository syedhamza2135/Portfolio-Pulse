...
export default function Dashboard() {
  const { user } = useAuth();
  const { data: portfolios, loading, error, refetch } = useFetch("/portfolios");

  // Calculate dashboard statistics
  const stats = useMemo(() => {
    if (!portfolios || loading) {
      return { totalPortfolios: 0, totalHoldings: 0 };
    }
    
    return {
      totalPortfolios: portfolios.length,
      // Note: If holdings aren't populated in the /portfolios endpoint,
      // this will always be 0. Consider fetching from /portfolios/stats endpoint
      totalHoldings: portfolios.reduce((sum, p) => sum + (p.holdings?.length || 0), 0),
    };
  }, [portfolios, loading]);

  // Get most recent portfolios
  const recentPortfolios = useMemo(() => {
    if (!portfolios) return [];
    return portfolios.slice(0, MAX_RECENT_PORTFOLIOS);
  }, [portfolios]);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {portfolios && portfolios.length > MAX_RECENT_PORTFOLIOS && (
            <Link to="/portfolios">
...
function SummaryCard({ label, value, icon, iconBgColor, iconColor }) {
  return (
