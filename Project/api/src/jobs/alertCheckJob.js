...
class AlertChecker {
  ...
  async checkAllAlerts() {
    ...
    try {
      ...
      }).select('_id email preferences').lean();
      ...
      for (const user of users) {
        try {
          ...
          for (const portfolio of portfolios) {
            ...
            const portfolioAlertSent = await this.checkPortfolioThreshold(user, portfolio);
            if (portfolioAlertSent) portfolioAlertsTriggered++;

            // Check individual holdings
            const holdingAlerts = await this.checkHoldingThresholds(user, portfolio);
          ...
          console.error(`[AlertJob] Error checking alerts for user ${user._id}:`, error.message);
        }
      }

      console.log(`[AlertJob] ✓ Complete. Portfolio alerts: ${portfolioAlertsTriggered}, Holding alerts: ${holdingAlertsTriggered}`);
  ...
  async checkPortfolioThreshold(user, portfolio) {
    try {
      const threshold = user.preferences?.alertThreshold || 3; // Default 3% per PRD
      
      // Calculate actual portfolio change
      ...
      const changePercent = ((currentValue - totalInvestment) / totalInvestment) * 100;

      // Check if threshold exceeded
      if (Math.abs(changePercent) >= threshold) {
        // Avoid duplicate alerts (check last alert time)
        const lastAlertKey = `portfolio_${portfolio._id}_${Math.floor(Date.now() / (60 * 60 * 1000))}`; // Hourly key
      ...
      console.error('[AlertJob] Portfolio threshold check error:', error.message);
      return false;
    }
  }
  ...
  async checkHoldingThresholds(user, portfolio) {
    try {
      ...
      for (const holding of holdings) {
        ...
        const changePercent = ((holding.currentPrice - holding.averageCost) / holding.averageCost) * 100;
